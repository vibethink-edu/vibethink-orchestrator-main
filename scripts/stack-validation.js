...
class StackValidator {
   ...
   */
  async validate() {
    console.log(`${colors.bold}${colors.blue}🔍 Validando Documento Maestro de Stack...${colors.reset}\n`);
  ...
  printResults(results) {
    console.log(`${colors.bold}${colors.blue}📊 Resultados de Validación:${colors.reset}\n`);
    ...
    Object.keys(results).forEach(key => {
      if (typeof results[key] === 'object' && results[key].total !== undefined) {
        ...
        console.log(`${status} ${key}: ${result.passed}/${result.total} (${percentage}%)`);
    ...
    if (results.failed > 0) {
      ...
      console.log('1. Actualizar documento maestro de stack');
      console.log('2. Revisar evaluaciones desactualizadas');
      console.log('3. Validar ADRs pendientes');
      console.log('4. Ejecutar validación nuevamente');
    }
  }
}
...
if (require.main === module) {
  ...
  validator.validate().then(success => {
    ...
    console.error(`${colors.red}❌ Error en validación: ${error.message}${colors.reset}`);
    process.exit(1);
  });
}
