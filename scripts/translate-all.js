#!/usr/bin/env node
/**
 * Translate All - Batch Translation Script
 *
...
if (!process.env.ANTHROPIC_API_KEY) {
  ...
  console.error(colorize('  ANTHROPIC_API_KEY=sk-ant-api03-...', 'cyan'));
  console.error('');
  process.exit(1);
}
...
console.log(colorize('  Batch Translation Tool - Projects V2', 'bright'));
console.log(colorize('='.repeat(79), 'cyan'));
console.log('');
...
async function translateOne(namespace, locale) {
  return new Promise((resolve, reject) => {
    const scriptPath = path.join(__dirname, 'translate-namespace.js');
