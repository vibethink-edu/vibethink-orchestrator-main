...
function findFiles(dir, extensions = ['.tsx', '.ts', '.js', '.jsx']) {
  ...
  function traverse(currentDir) {
    try {
      ...
      console.log(`⚠️  Error accediendo a ${currentDir}: ${error.message}`);
    }
  }
  ...
  try {
    ...
    Object.entries(importMappings).forEach(([oldImport, newImport]) => {
      ...
      if (newContent !== content) {
        content = newContent;
        hasChanges = true;
        console.log(`✅ Corregida importación en ${filePath}: ${oldImport} → ${newImport}`);
      }
    });
...
function main() {
  ...
  console.log(`   Archivos sin cambios: ${files.length - fixedFiles}`);
