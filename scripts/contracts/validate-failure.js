...
function validateFailure(payload, schema) {
  const errors = [];
  validateSchema(schema, payload, "failure", errors);
  return errors;
}
...
function main() {
  ...
  if (!targetPath) {
    console.error("Usage: node validate-failure.js <file.json>");
    process.exit(2);
  }

  const repoRoot = findRepoRoot(__dirname);
  const schemaPath = path.join(repoRoot, "schemas", "runtime", "failure-envelope.schema.json");
  const schema = loadJson(schemaPath);
  const payload = loadJson(path.resolve(targetPath));

  const errors = validateFailure(payload, schema);
  if (errors.length > 0) {
    console.error("FailureEnvelope validation failed:");
    errors.forEach((error) => console.error(`- ${error}`));
    process.exit(1);
  }

  console.log("FailureEnvelope validation passed.");
}
