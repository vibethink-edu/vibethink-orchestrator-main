# Start Dashboard-Tenant (Client/Company Admin)
# Port: 3007
# Purpose: Admin para empresas clientes - Multi-tenant

Write-Host "`n👥 Starting Dashboard-Tenant (Client Admin)..." -ForegroundColor Cyan
Write-Host "Purpose: Client/Company Admin Dashboard" -ForegroundColor Yellow
Write-Host "Port: 3007`n" -ForegroundColor Green
...
if ($connections) {
    ...
    if ($validProcessIds.Count -gt 0) {
        ...
        try {
            $stopScript = Join-Path $PSScriptRoot "stop-dashboard-tenant.ps1"
...
Write-Host "🚀 Starting Dashboard-Tenant on port $port..." -ForegroundColor Green
...
$job = Start-Job -ScriptBlock {
    ...
    Start-Process "http://localhost:3007/dashboard-tenant/overview"
}
