...
class PorteUpdateTracker {
  ...
  loadPorteConfig() {
    try {
      ...
      console.error('Error loading porte config:', error);
      return [];
    }
  }
  ...
  async checkAllUpdates() {
    console.log('🔍 Checking for updates on ported projects...\n');
    ...
    for (const project of this.porteConfig) {
      ...
      try {
        ...
        if (updateInfo.hasUpdate) {
          ...
          console.log(`  ℹ️  No updates available (current: ${updateInfo.currentVersion})`);
        }
      } catch (error) {
   ...
   */
  async run() {
    console.log('🚀 Starting AI-Pair Porte Update Tracker...\n');
    ...
    console.log(`  Projects with Updates: ${report.summary.projectsWithUpdates}`);
    console.log(`  Projects with Errors: ${report.summary.projectsWithErrors}`);
  ...
  switch (command) {
    ...
    case 'analyze':
      ...
      if (projectName && targetVersion) {
        ...
        console.log('Usage: node porte-update-tracker.js analyze [PROJECT_NAME] [TARGET_VERSION]');
      }
      break;
    case 'report':
      ...
      break;
    default:
      console.log('AI-Pair Porte Update Tracker');
      console.log('Usage:');
      console.log('  node porte-update-tracker.js check     - Check all projects for updates');
      console.log('  node porte-update-tracker.js analyze   - Analyze update impact');
      console.log('  node porte-update-tracker.js report    - Generate update report');
      break;
  }
}
