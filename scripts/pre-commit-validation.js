...
class PreCommitValidator {
   ...
   */
  async run() {
    console.log(`${colors.bold}${colors.blue}🔍 Pre-commit Validation: Stack Evaluation Criteria${colors.reset}\n`);
    ...
    try {
      ...
      if (evaluationFiles.length === 0) {
        console.log(`${colors.green}✅ No se encontraron archivos de evaluación modificados${colors.reset}`);
        return true;
      }

      console.log(`${colors.cyan}📁 Archivos de evaluación modificados:${colors.reset}`);
      ...
      console.error(`${colors.red}Error en validación pre-commit: ${error.message}${colors.reset}`);
      return false;
    }
  }
  ...
  getModifiedFiles() {
    try {
      ...
      console.warn(`${colors.yellow}Warning: No se pudo obtener archivos modificados: ${error.message}${colors.reset}`);
      return [];
    }
  }
  ...
  async validateFile(filePath) {
    console.log(`${colors.cyan}🔍 Validando: ${filePath}${colors.reset}`);
    ...
    try {
      ...
      if (extension === '.json') {
        ...
        console.log(`  ${colors.yellow}⚠️  Formato no soportado: ${extension}${colors.reset}`);
      ...
      if (isValid) {
        console.log(`  ${colors.green}✅ Validación pasó${colors.reset}`);
      } else {
        console.log(`  ${colors.red}❌ Validación falló${colors.reset}`);
      }
      
      return isValid;
      
    } catch (error) {
      console.error(`  ${colors.red}Error validando ${filePath}: ${error.message}${colors.reset}`);
      return false;
    }
  }
  ...
  validateEvaluationData(evaluationData, filePath) {
    ...
    if (missingCriteria.length > 0) {
      console.log(`  ${colors.red}Criterios faltantes:${colors.reset}`);
    ...
    for (const [criterion, items] of Object.entries(evaluationData)) {
      if (this.requiredCriteria.includes(criterion)) {
        ...
        if (items.length < minItems) {
          console.log(`  ${colors.red}${criterion}: Insuficientes items (${items.length}/${minItems})${colors.reset}`);
          isValid = false;
        } else {
          console.log(`  ${colors.green}${criterion}: ${items.length} items completados${colors.reset}`);
        }
      }
    }
  ...
  showSummary(allValid, fileCount) {
    ...
    if (allValid) {
      console.log(`${colors.green}${colors.bold}🎉 TODAS LAS EVALUACIONES PASARON${colors.reset}`);
      console.log(`${colors.green}✅ ${fileCount} archivo(s) validado(s) correctamente${colors.reset}`);
      console.log(`${colors.green}✅ Commit puede proceder${colors.reset}`);
    } else {
      console.log(`${colors.red}${colors.bold}⚠️  VALIDACIÓN FALLÓ${colors.reset}`);
      console.log(`${colors.red}❌ Algunas evaluaciones no cumplen con los criterios obligatorios${colors.reset}`);
      console.log(`${colors.yellow}💡 Corrige las evaluaciones antes de hacer commit${colors.reset}`);
      console.log(`${colors.yellow}💡 Usa: node scripts/validate-stack-evaluation.js help${colors.reset}`);
    }
...
async function main() {
  ...
  if (!success) {
    console.log(`${colors.red}${colors.bold}❌ Pre-commit validation failed${colors.reset}`);
    process.exit(1);
  }
  
  console.log(`${colors.green}${colors.bold}✅ Pre-commit validation passed${colors.reset}`);
}
