...
class VersionAutomation {
  ...
  loadVersioningConfig() {
    try {
      ...
      console.error('Error loading versioning config:', error);
      return {
  ...
  updateVersioningConfig(newVersion) {
    try {
      ...
      console.log(`✅ Updated versioning config to v${newVersion}`);
   ...
   */
  async run() {
    console.log('🚀 Starting AI-Pair Version Automation...\n');
    
    // Detecta cambios
    const changeInfo = this.detectChanges();
    if (!changeInfo) {
      console.log('❌ No changes detected or error occurred');
      return false;
    }
    
    console.log(`📝 Detected changes:`);
    console.log(`   Author: ${changeInfo.author}`);
    console.log(`   Subject: ${changeInfo.subject}`);
    console.log(`   Type: ${changeInfo.changeType}`);
    console.log(`   Files: ${changeInfo.modifiedFiles.length} modified\n`);
    
    // Determina nueva versión
    const newVersion = this.updateVersion(changeInfo.changeType);
    console.log(`🔄 Updating version: ${this.versioningConfig.currentVersion} → ${newVersion}\n`);
    
    // Actualiza configuración
    const configUpdated = this.updateVersioningConfig(newVersion);
    if (!configUpdated) {
      console.log('❌ Failed to update versioning config');
      return false;
    }
    ...
    console.log('✅ Version automation completed successfully!');
    console.log(`📊 New version: v${newVersion}`);
    console.log(`👥 Team: ${this.getTeamMember(changeInfo.author)}`);
  ...
  generateReport() {
    try {
      ...
      console.log(`📊 Version report generated: ${reportPath}`);
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    case 'detect':
      ...
      break;
    default:
      console.log('AI-Pair Version Automation');
      console.log('Usage:');
      console.log('  node version-automation.js run     - Run full automation');
      console.log('  node version-automation.js report  - Generate version report');
      console.log('  node version-automation.js detect  - Detect changes only');
      break;
  }
}
