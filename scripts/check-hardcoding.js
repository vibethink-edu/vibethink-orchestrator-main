...
class HardcodingDetector {
  ...
  displayReport(report) {
    ...
    console.log(`   Altas: ${report.summary.high} ⚠️`);
    ...
    if (report.violations.high.length > 0) {
      ...
      report.violations.high.forEach((v, index) => {
        console.log(`   ${index + 1}. ${v.file}:${v.line} - ${v.message}`);
      });
    }
    ...
    if (report.recommendations.length > 0) {
      ...
      report.recommendations.forEach((r, index) => {
        console.log(`   ${index + 1}. ${r.priority}: ${r.message}`);
    ...
    if (report.summary.critical > 0) {
      console.log('🚨 RESULTADO: COMMIT BLOQUEADO - Corrige las violaciones críticas');
      return false; // Indica que hay violaciones críticas
    } else if (report.summary.high > 0) {
      console.log('⚠️  RESULTADO: ADVERTENCIAS - Considera corregir las violaciones altas');
      return true; // Permite commit pero con advertencias
    } else {
      console.log('✅ RESULTADO: SIN VIOLACIONES CRÍTICAS - Commit permitido');
...
async function main() {
  ...
  console.log('🔍 AI Pair Hardcoding Detector v1.0.0');
  console.log('=====================================');
