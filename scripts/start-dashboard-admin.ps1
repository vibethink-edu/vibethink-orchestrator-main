# Start Dashboard-Admin (VibeThink Internal Admin)
# Port: 3006
# Purpose: Admin interno de VibeThink - Gestión de clientes y sistema

Write-Host "`n🔧 Starting Dashboard-Admin (VibeThink Internal)..." -ForegroundColor Cyan
Write-Host "Purpose: Internal Admin - Tenant Management" -ForegroundColor Yellow
Write-Host "Port: 3006`n" -ForegroundColor Green
...
if ($connections) {
    ...
    if ($validProcessIds.Count -gt 0) {
        ...
        try {
            $stopScript = Join-Path $PSScriptRoot "stop-dashboard-admin.ps1"
...
Write-Host "🚀 Starting Dashboard-Admin on port $port..." -ForegroundColor Green
...
$job = Start-Job -ScriptBlock {
    ...
    Start-Process "http://localhost:3006/dashboard-admin/tenants"
}
