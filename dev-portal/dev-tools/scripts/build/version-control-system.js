...
class VersionControlSystem {
  ...
  validateVersion(integration, version, environment) {
    ...
    if (certification.level !== requiredLevel && environment === 'prod') {
      validation.isValid = false;
      validation.errors.push(`Versión ${version} no tiene nivel de certificación requerido para ${environment}`);
    }
    ...
    if (versionAge > rules.maxAge) {
      validation.warnings.push(`Versión ${version} tiene ${versionAge} días, máximo recomendado: ${rules.maxAge} días`);
    }
  ...
  validateConfigurationFile(configPath) {
    console.log(`🔍 Validando configuración: ${configPath}`);
  ...
  generateValidationReport(results) {
    ...
    console.log(`   Válidas: ${results.valid} ✅`);
    console.log(`   Inválidas: ${results.invalid} ❌`);
    console.log(`   Advertencias: ${results.warnings} ⚠️`);
  ...
  validateCertificationProcess() {
    ...
    for (const [integration, version] of Object.entries(prodVersions)) {
      ...
      if (!certification) {
        ...
        processValidation.errors.push(`${integration} ${version} no está certificada para producción`);
      } else if (certification.level !== 'certified') {
        processValidation.isValid = false;
        processValidation.errors.push(`${integration} ${version} no tiene nivel 'certified' para producción`);
      }
    }
