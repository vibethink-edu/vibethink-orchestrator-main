...
async function setupTestData(supabase: any) {
  ...
  // Setup usage data
  console.log('📊 Setting up test usage data...');
  await setupUsageData(supabase, testData.usageData);

  // Setup billing data
  console.log('💰 Setting up test billing data...');
...
async function setupCompanies(supabase: any, companies: any[]) {
  for (const company of companies) {
    ...
    if (error) {
      console.warn(`⚠️ Warning: Could not setup company ${company.slug}:`, error.message);
    } else {
...
async function setupUsers(supabase: any, users: any[]) {
  for (const user of users) {
    ...
    if (error) {
      console.warn(`⚠️ Warning: Could not setup user ${user.email}:`, error.message);
    } else {
...
async function setupConfigurations(supabase: any, configurations: any[]) {
  for (const config of configurations) {
    ...
    if (error) {
      console.warn(`⚠️ Warning: Could not setup configuration ${config.config_key}:`, error.message);
    } else {
...
async function setupUsageData(supabase: any, usageData: any[]) {
  for (const usage of usageData) {
    ...
    if (error) {
      console.warn(`⚠️ Warning: Could not setup usage data:`, error.message);
    } else {
      console.log(`✅ Created usage record for ${usage.user_id}`);
    }
  }
}
...
async function setupBillingData(supabase: any, billingData: any[]) {
  for (const billing of billingData) {
    ...
    if (error) {
      console.warn(`⚠️ Warning: Could not setup billing data:`, error.message);
    } else {
      console.log(`✅ Created billing record for ${billing.company_id}`);
    }
  }
}
