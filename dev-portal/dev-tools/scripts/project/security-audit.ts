...
async function runSecurityAudit(companyId: string): Promise<SecurityAuditResult> {
  console.log(`🔒 Iniciando auditoría de seguridad para empresa: ${companyId}`);
...
async function main() {
  ...
  if (!companyId) {
    console.error('❌ Error: Debe proporcionar un company_id');
  ...
  try {
    ...
    if (failedChecks.length > 0) {
      ...
      failedChecks.forEach(check => {
        ...
        console.log(`  ${statusIcon} [${check.severity.toUpperCase()}] ${check.name}`);
        console.log(`     ${check.details}`);
        if (check.recommendation) {
          console.log(`     💡 Recomendación: ${check.recommendation}`);
        }
      });
    }
    ...
    if (result.overallScore >= 90) {
      console.log('🌟 Excelente! La seguridad está muy bien configurada.');
    } else if (result.overallScore >= 70) {
      console.log('👍 Bueno! Hay algunas mejoras menores que hacer.');
    } else if (result.overallScore >= 50) {
      console.log('⚠️ Regular! Se requieren mejoras importantes.');
    } else {
