# 🤖 Script de Inicialización AIPAIR para PowerShell
...
function New-AIPAIRInitCommand {
    param(
        [string]$Objective,
        [string]$PendingTasks,
        [string]$UrgentIssues
...
- **Rol:** CEO/CTO
- **Email:** marcelo@aipair.com
- **Empresa:** AI-Pair Orchestrator Pro
...
function Show-Help {
    Write-Host @"
🤖 Script de Inicialización AIPAIR
================================

Uso:
    .\init-aipair-session.ps1 [parámetros]
...
Ejemplos:
    .\init-aipair-session.ps1 -Objective "Desarrollo de nueva funcionalidad"
    .\init-aipair-session.ps1 -Objective "Revisión de código" -PendingTasks "Test unitarios, Documentación"
    .\init-aipair-session.ps1 -Objective "Debugging" -UrgentIssues "Error en producción"

Comandos rápidos:
    .\init-aipair-session.ps1 -Objective "Desarrollo"
    .\init-aipair-session.ps1 -Objective "Testing"
    .\init-aipair-session.ps1 -Objective "Documentación"
"@ -ForegroundColor Cyan
}

# Función principal
function Start-AIPAIRSession {
    param(
        [string]$Objective,
        [string]$PendingTasks,
        [string]$UrgentIssues
    ...
    Write-Host "🤖 Inicializando sesión AIPAIR..." -ForegroundColor Green
    ...
    $initCommand = New-AIPAIRInitCommand -Objective $Objective -PendingTasks $PendingTasks -UrgentIssues $UrgentIssues
    ...
    Write-Host "2. AIPAIR ejecutará el protocolo de inicialización" -ForegroundColor White
...
function New-AIPAIRAlias {
    Write-Host "🔧 Creando alias para AIPAIR..." -ForegroundColor Yellow
    
    $aliasScript = @"
# Alias para AIPAIR
function aipair {
    param([string]`$Objective = "")
    & "$PSScriptRoot\init-aipair-session.ps1" -Objective `$Objective
}

# Exportar función
Export-ModuleMember -Function aipair
"@
    ...
    Write-Host "🔄 Reinicia PowerShell para usar el comando 'aipair'" -ForegroundColor Yellow
}

# Función para mostrar estado del sistema
function Show-SystemStatus {
    Write-Host "🔍 Estado del Sistema AIPAIR" -ForegroundColor Cyan
    ...
    # Verificar snippets
    $snippetsPath = "$env:USERPROFILE\.cursor\snippets\aipair-init.code-snippets"
    if (Test-Path $snippetsPath) {
        Write-Host "✅ Snippets de AIPAIR configurados" -ForegroundColor Green
    } else {
        Write-Host "❌ Snippets de AIPAIR no encontrados" -ForegroundColor Red
    }
...
Start-AIPAIRSession -Objective $Objective -PendingTasks $PendingTasks -UrgentIssues $UrgentIssues 