...
CREATE TABLE IF NOT EXISTS companies (
  ...
  plan_type TEXT DEFAULT FREE CHECK (plan_type IN ('FREE','BASIC','PRO,'ENTERPRISE)),  plan_limits JSONB DEFAULT '[object Object]tatus TEXT DEFAULT 'ACTIVE CHECK (status IN ('ACTIVE',SUSPENDED,LLED')),
...
CREATE TABLE IF NOT EXISTS tasks (
  ...
  title TEXT NOT NULL,
  description TEXT,
  status TEXT DEFAULT 'TODO CHECK (status IN ('TODO','IN_PROGRESS',REVIEW','DONE,LED')),
  priority TEXT DEFAULT 'MEDIUM' CHECK (priority IN ('LOW','MEDIUM,GH','CRITICAL)),  assignee_id UUID REFERENCES users(id) ON DELETE SET NULL,
  assignee_name TEXT,
  due_date TIMESTAMP WITH TIME ZONE,
  estimated_hours DECIMAL(5,2  actual_hours DECIMAL(5,2),
  tags TEXTULT '[object Object]ource TEXT DEFAULT 'manual CHECK (source IN ('manual','dartai,eting','email')),
  source_id TEXT,
...
CREATE TABLE IF NOT EXISTS workflow_executions (
  ...
  status TEXT DEFAULTRUNNING CHECK (status IN (RUNNING','COMPLETED','FAILED,CELLED')),
...
CREATE TABLE IF NOT EXISTS meetings (
  ...
  transcript TEXT,
  summary TEXT,
  action_items JSONB DEFAULT 'tatus TEXT DEFAULTSCHEDULED CHECK (status IN ('SCHEDULED','IN_PROGRESS',COMPLETED,LED')),
  metadata JSONB DEFAULT {},
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  company_id UUID REFERENCES companies(id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS resources (
  ...
  title TEXT NOT NULL,
  url TEXT,
  type TEXT CHECK (type IN ('article','video','document,bsite','other')),
  content TEXT,
  summary TEXT,
  tags TEXTT '{},
  metadata JSONB DEFAULT {},
...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  title TEXT NOT NULL,
  message TEXT,
  type TEXT DEFAULT INFO' CHECK (type IN (INFO',SUCCESS,RNING','ERROR)), read_at TIMESTAMP WITH TIME ZONE,
  action_url TEXT,
  metadata JSONB DEFAULT {},
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  company_id UUID REFERENCES companies(id) ON DELETE CASCADE
...
CREATE POLICY Admins can manage users in their company" ON users
  FOR ALL USING (
    ...
    AND auth.jwt() ->>roleIN ('ADMIN','OWNER,PER_ADMIN));
