...
class DependencyValidator {
    ...
    for (const [name, version] of Object.entries(deps)) {
      ...
      if (hasCaretOrTilde) {
        if (appType === 'core') {
          ...
          this.success.push(`✅ ${filePath} ${section}["${name}"]: ${version} (reference app - mantiene versiones originales)`);
        }
  ...
  getAppType(filePath) {
    ...
    const appName = filePath.split(/[\\/]/).find(segment =>
      coreApps.includes(segment) || marketingApps.includes(segment) || referenceApps.includes(segment)
    );
