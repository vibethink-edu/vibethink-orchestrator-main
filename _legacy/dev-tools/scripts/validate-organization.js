...
`;
  ...
  run() {
    ...
    console.log(`📍 Estructura raíz: ${this.violations.filter(v => v.includes('raíz')).length === 0 ? '✅' : '❌'}`);
    console.log(`📁 Dev-tools: ${this.warnings.filter(w => w.includes('dev-tools')).length === 0 ? '✅' : '⚠️'}`);
    console.log(`📊 Reportes: ${this.violations.filter(v => v.includes('reportes')).length === 0 ? '✅' : '❌'}`);
    console.log(`📝 Nomenclatura: ${this.warnings.filter(w => w.includes('nomenclatura')).length === 0 ? '✅' : '⚠️'}`);
