          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <div className="space-y-1 text-xs">
                ...
                <div><strong>Last:</strong> <code className="text-xs break-all">{systemVars.errors.lastError}</code></div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-1 text-xs">
                <div><strong>Runtime:</strong> {systemVars.debugging.errorTypes.runtime}</div>
                <div><strong>Promises:</strong> {systemVars.debugging.errorTypes.promises}</div>
                <div><strong>Sources:</strong> {systemVars.debugging.errorSources.length} files</div>
              </div>
            </div>
          </div>
                      ...
                      {errors.slice(0, 3).map((error, index) => (
                        <div key={index} className="bg-red-50 dark:bg-red-900/20 p-2 rounded text-xs border border-red-200 dark:border-red-800">
                          <div className="flex justify-between items-start mb-1">
                            ...
                            <Badge variant="outline" className="text-xs ml-2">{error.timestamp.split('T')[1]?.split('.')[0]}</Badge>
                          </div>
                          <div className="text-gray-600 dark:text-gray-400 text-xs">
                            📁 {error.source.split('/').pop() || 'Unknown'}
                          </div>
                          {error.stack && (
                            <details className="mt-1">
                              <summary className="cursor-pointer text-xs text-gray-500">Stack trace</summary>
                              <pre className="text-xs mt-1 bg-gray-100 dark:bg-gray-800 p-1 rounded overflow-x-auto">
                                {error.stack.split('\n').slice(0, 3).join('\n')}
                              </pre>
                            </details>
                          )}
                    ...
                    <div className="space-y-1">
                      {consoleErrors.slice(0, 3).map((error, index) => (
                        <div key={index} className="bg-orange-50 dark:bg-orange-900/20 p-2 rounded text-xs border border-orange-200 dark:border-orange-800">
                          <code className="text-orange-700 dark:text-orange-300 font-mono text-xs break-all">
